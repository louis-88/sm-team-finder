...
public class SafeToPlayLookupService implements PermissionService {
    ...
    private Boolean function(PermissionRequest permissionRequest) {
        ...
        if (appConfig.getSafetyCheck().contains(permissionRequest.getPlayer())) {
            ...
            String connectionUrl = "jdbc:sqlserver://steembot.database.windows.net:1433;database=SteemMonsters;encrypt=true;trustServerCertificate=false;hostNameInCertificate=*.database.windows.net;loginTimeout=30;user=steembotuser;password=gXslR1r6WTGEGSyQSUDn";
            try {
